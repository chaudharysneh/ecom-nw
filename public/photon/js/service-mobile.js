...
$saveAndContinue.on('click', function() {
  ...
  if (supportingFileAPI) {
    ...
    //var canvas = $('.tui-image-editor-canvas-container canvas.lower-canvas')[0];
    // Create a FormData object
    var formData = new FormData();
    formData.append('image', blob, imageName);
    formData.append('productId',productId);
    // Get the Fabric.js canvas JSON representation
...
function loadImage($imgurl){
  imageEditor.loadImageFromURL($imgurl, 'SampleImage').then(function () {
    imageEditor.clearUndoStack();
  });
}

function saveAndContinue($url, $ProductID) {
  var imageName = imageEditor.getImageName();
  var dataURL = imageEditor.toDataURL();
  var blob, type, w;
  if (supportingFileAPI) {
...
uploadcontainer.addEventListener('click', function(event) {
  // Check if the clicked element has the "delet_image" class
  if (event.target.classList.contains('delet_image')) {
    const templateId = event.target.getAttribute('data-template_id');
    //alert(templateId);
    // Create an AJAX request
    const xhr = new XMLHttpRequest();
    xhr.open('POST', '/delete-template', true);
    ...
    // Send the AJAX request with the template_id as data
    xhr.send(`template_id=${templateId}`);
  }
});
